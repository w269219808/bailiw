...
def getJson():
    ...
    sellerId = input('请输入爬取的sellerId:')
    global foodsId
    foodsId = str(sellerId)
   # start_url = 'https://s.taobao.com/search?q={}&sort=sale-desc&s='.format(name)
#    t = datetime.date.today() - datetime.timedelta(days=1)  #获取最近30天的数据
    t = date.today() - timedelta(days=1)

    #t = '2020-06-21'  #定义找查的截至时间
    shop_url ='https://sycm.taobao.com/flow/v3/monitor/shop/trend.json?dateType=day&dateRange={}%7C{}&cateId=0&sellerId={}&device=0'.format(t,t,sellerId)
    ...
    for cookie in mycookies:
        ...
        cookies[name] = value
    html = ''
    r = requests.get(shop_url,headers=header,cookies=cookies,timeout=60)
...
def trans(html):
    ...
    index = []
    t = date.today() - timedelta(days=31)
